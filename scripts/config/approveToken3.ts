...
async function main() {
  ...
  const msgHash = encode.addHexPrefix(
    hash.hashMessage(
      ...
      addresses.alpha.token3,
      stark.getSelectorFromName("approve"),
      [addresses.alpha.router, pool.token3Balance, "0"],
      nonce.toString()
    )
  );
  ...
  const { code, transaction_hash } = await account.invoke(
    "execute",
    {
      to: addresses.alpha.token3,
      selector: stark.getSelectorFromName("approve"),
      calldata: [addresses.alpha.router, pool.token3Balance, "0"],
      nonce: nonce.toString(),
    },
    [number.toHex(r), number.toHex(s)]
  );

  console.log(`Transaction hash approve tx for token 3: ${transaction_hash}`);
  console.log(`Transaction status approve tx for token 3: ${code}`);
  console.log(
    "get new status approve tx for token 3: ",
