...
async function main() {
  ...
  const { code, transaction_hash } = await account.invoke(
    "execute",
    {
      ...
      calldata: [addresses.alpha.adminAccount, ""],
      nonce: nonce.toString(),
    },
    [number.toHex(r), number.toHex(s)]
  );

  console.log(`Transaction hash token0: ${transaction_hash}`);
  console.log(`Transaction status token0: ${code}`);
  console.log(
    "get new status token0: ",
