...
async function main() {
  ...
  const msgHash = encode.addHexPrefix(
    hash.hashMessage(
      ...
      [addresses.alpha.router, pool.token4Balance, "0"],
      nonce.toString()
    )
  );
  ...
  const { code, transaction_hash } = await account.invoke(
    "execute",
    {
      to: addresses.alpha.token4,
      selector: stark.getSelectorFromName("approve"),
      calldata: [addresses.alpha.router, pool.token4Balance, "0"],
      nonce: nonce.toString(),
    },
    [number.toHex(r), number.toHex(s)]
  );

  console.log(`Transaction hash approve tx for token 4: ${transaction_hash}`);
  console.log(`Transaction status approve tx for token 4: ${code}`);
  console.log(
    "get new status approve tx for token 4: ",
